...
function normalize(raw) {
  ...
  Object.entries(raw).forEach(([k, v]) => {
    STOCKS[k] = {
      name: v.n, group: v.g, accent: v.a, currency: v.cur, currencySymbol: v.cs, note: v.nt || null,
      segments: v.seg, segmentColors: v.sc, geo: v.geo,
      quarterly: v.q.map(r => ({ q: r[0], segments: r[1], total: r[2], oi: r[3] })),
      sankeyData: v.sk ? {
        year: v.sk.y, totalRevenue: v.sk.r,
        segments: v.sk.s.map(([n, val]) => ({ name: n, value: val })),
        costs: v.sk.c.map(([n, val]) => ({ name: n, value: val })),
        operatingIncome: v.sk.oi
      } : null,
    };
  });
  const GT = {};
  Object.keys(GROUPS).forEach(g => { GT[g] = Object.keys(STOCKS).filter(t => STOCKS[t].group === g) });
  return { STOCKS, GT };
}
  ...
  return<div style={{fontFamily:ff,background:t.bg,color:t.tx,minHeight:"100vh",padding:"24px 20px"}}>
    ...
    {tab===3&&<div>
      ...
      {geoEntries.length>0?<div>
        ...
        <div style={{fontSize:11,color:t.tf,marginTop:8}}>Geographic splits are annual estimates based on latest filings.</div>
